...
- 图鉴按键：列表长按上/下跳 10，长按确定切换中英（不再回演示菜单）。详情
  长按上/下缩放立绘，双击上/下跳 10，确定叫声，双击翻译，长按回列表。
- 图鉴详情页立绘按官方 48×48 画布最近邻 3 倍（144px，相对原先 2 倍再放大
  1.5 倍），不裁切。立绘区与概述约 2/3 : 1/3。顶边左编号、中名字、右属性
  （双属性纵向叠），底边左身高、右体重；概述仍可自动慢滚。
- 图鉴详情页精灵井改为顶栏下居中 144×144：先裁掉 48×48 精灵四周的透明边，
  再最近邻放大铺满，避免立绘仍缩在大框中间。14px 中文字库改用 Hiragino Sans GB W6 并轻微加粗，深色底上
  不再发虚；ASCII 一并打进同一字库，数字不再回落到偏细的 Montserrat。
- 图鉴查询改为列表 + 查找：开机进当前世代列表（单击翻 1、双击按英文首字母
  跳到下一组、长按上/下打开查找）。查找页可跳号、按 A–Z 选名字，或进九世代；
  跳号用四位数字输入。详情页仍是精灵图与概述（确定播放叫声）。列表页长按
  确定回菜单，其它页长按确定退回列表。确定双击切换中/英文（名字、概述、界面），
  选择写入 NVS。串口 `FAP_POKEDEX_LANG` 只预览语言，不覆盖已保存的选择。
  中文 4bpp 字体按行对齐，避免奇数宽汉字花屏。串口
  `FAP_POKEDEX_VIEW` / `FAP_POKEDEX_LANG` / `FAP_POKEDEX_ID` 供截图调试。
- 开机直接进入离线宝可梦图鉴（全国图鉴 `1..1025`，内嵌 48×48 精灵图）。
  长按确定仍返回演示菜单，图鉴现为第一项。
